...
var asDateRequirement = (type, requirement, context) => {
...
function convertPipe(schemaType, pipe, context) {
  return pipe.reduce((def, validation) => {
    ...
    if (!validationConverter && context.ignoreUnknownValidation)
      return {};
    assert(validationConverter, Boolean, `Unsupported valibot validation \`${validationType}\` for schema \`${schemaType}\``);
    return Object.assign(def, validationConverter(validation, context));
  }, {});
}
...
function createConverter(context) {
  ...
  function converter(schema) {
    ...
    let converted;
    const customConverter = (_a = context.customSchemaConversion) == null ? void 0 : _a[schema.type];
    if (customConverter) {
      converted = customConverter(schema, converter, context);
      assert(converted, Boolean, `Custom schema converter \`${schema.type}\` returned an invalid JSON schema`);
    } else {
      const schemaConverter = ((_b = context.customSchemaConversion) == null ? void 0 : _b[schema.type]) || SCHEMA_CONVERTERS[schema.type];
      assert(schemaConverter, Boolean, `Unsupported valibot schema: ${(schema == null ? void 0 : schema.type) || schema}`);
      converted = schemaConverter(schema, converter, context);
    }
    Object.assign(converted, convertPipe(schema.type, schema.pipe || [], context));
